...
void CalibratorApp::changeTempState(enum TempState NextState)
{
  ...
  case TEMP_STATE_WSETUP:
    {
      m_TwiBuffer[0] = 0x02; /* register 2: configuration */
      m_TwiBuffer[1] = 0x10;
      m_TwiBuffer[2] = 0x00;

      twi.sendStartAndWrite(HDC1000_ADDRESS, m_TwiBuffer, 3);
      Globals::getGlobals().getTemperatureTimer().startSeconds(5);
    }
    break;
  case TEMP_STATE_WTRIGGER:
    {
      m_TwiBuffer[0] = 0x00; /* register 0: trigger */
      twi.sendStartAndWrite(HDC1000_ADDRESS, m_TwiBuffer, 1);
      Globals::getGlobals().getTemperatureTimer().startSeconds(5);
    }
    break;
  case TEMP_STATE_WMEASUREMENT:
    {
      twi.sendStartAndRead(HDC1000_ADDRESS, m_TwiBuffer, 4);
    }
    break;
  case TEMP_STATE_RETRYMEASUREMENT:
  ...
  if(TEMP_STATE_WTIMEOUT == NextState)
  {
       ...
       m_MenuState == UI_STATE_AUTOWAIT)
    {
      ...
      String::formatUnsigned(Buffer + 0, 3, ADC);
...
void CalibratorApp::handleTempState()
{
  ...
  case TEMP_STATE_WSETUP:
    {
      if (twi.STATUS_OK == status)
      {
        changeTempState(TEMP_STATE_WTRIGGER);
      }
      else if(twi.STATUS_BUSY == status)
      {
        /* still working */
  ...
  case TEMP_STATE_WMEASUREMENT:
    {
      if (twi.STATUS_NACK == status)
      {
        changeTempState(TEMP_STATE_RETRYMEASUREMENT);
      }
      else if(twi.STATUS_BUSY == status)
      {
        if(Globals::getGlobals().getTemperatureTimer().hasTimedOut())
